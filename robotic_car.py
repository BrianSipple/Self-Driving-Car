"""
Combines the robot class with control, movement, planning
and localization filtering functions to comprise the fundamental 
...
def run(grid, goal, spath, params, printflag = False, speed = 0.1, timeout = 1000):
    ...
    while not myrobot.check_goal(goal) and N < timeout:
        ...
        estimate = filter.get_position()	# get normalized [x, y, theta] from particle filter
...
if __name__ == "__main__":
	...
	params = twiddle([weight_data, weight_smooth, p_gain, d_gain])
	p_gain = params[2]
	d_gain = params[3]
	plan = plan(grid, init, goal)
	plan.astar()
	plan.smooth()	#TODO: check for possible error in smooth that muddles spath

	print run(grid, goal, plan.spath, [p_gain, d_gain], printflag = False, speed = 0.1, timeout = 1000)
